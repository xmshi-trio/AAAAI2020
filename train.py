...
class ClassificationTrainer(object):
    ...
    def run(self, data_loader, model, optimizer, stage,
        ...
        #relative2target_matrix = torch.load('/users3/xmshi/AAAI2020/data/relative2target.pt')
        total_loss = 0.
        for batch in data_loader:
            ...
            if self.conf.task_info.hierarchical:
                ...
                loss = self.loss_fn(
                    logits,
                    batch[ClassificationDataset.DOC_LABEL].to(self.conf.device))
                #target_priority = torch.mm(batch[ClassificationDataset.DOC_LABEL].to(self.conf.device), torch.sigmoid(relative2target_matrix.to(self.conf.device)))
                #target_priority = -torch.nn.Threshold(-0.5, -1)(-torch.nn.Threshold(0.5, 0)(target_priority))
                #loss = self.loss_fn(
                #    logits, target_priority)        
...
def train(conf):
    ...
    #model = torch.load('./checkpoint_dir_rcv1/relative_rule')
    ...
    for epoch in range(conf.train.start_epoch,
        ...
        if performance > best_performance:  # record the best model
            ...
            torch.save(model, './checkpoint_dir_rcv1/relative_rule')
        save_checkpoint({
            'epoch': epoch,
